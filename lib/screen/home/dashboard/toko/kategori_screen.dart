...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  _refreshData() {
    ...
    _futureDataCategories = TokoRepository().getAllCategories(token!).then((value) {
      log(value.toString());
      if (value.error != null) {
        showDialog(
            context: context,
      ...
      setState(() {
        ...
        categories.asMap().forEach((key, category) { 
          if(category.nama.toLowerCase().contains(_searchText.toLowerCase())){
            filteredCategories.add(
              Category(id: category.id, nama: category.nama)
            );
            switch (key) {
              case 0:
        ...
        ResultModel<List<Category>> result = ResultModel(
          isSuccess: true,
    ...
    return _futureDataShoppingCart = TokoRepository().getShoppingCart(token).then((value) {
      log(value.toString());
      if (value.error != null) {
        showDialog(
            context: context,
                                ...
                                return GestureDetector(
                                  onTap: () {
                                    ...
                                    if (links[categories.indexOf(category)]) {
                                      Navigator.of(context).pushNamed(
                                        TokoScreen.ROUTE_NAME, 
                                        arguments: TokoScreenArguments(categoryId: category.id)
                                      ).then((value) => _refreshData());
                                    } else {
                                      setState(() {
                                        links[categories.indexOf(category)] = true;
                                        print("link${categories.indexOf(category).toString()}" + ":" + links[categories.indexOf(category)].toString());
                                      });
                                    }
                                  },
                                  child: ExpandedButton(
                                    text: category.nama, 
                                    color1: filteredColor[categories.indexOf(category)][0], 
                                    color2: filteredColor[categories.indexOf(category)][1], 
                                    image1: filteredImage[categories.indexOf(category)][0], 
                                    image2: filteredImage[categories.indexOf(category)][1], 
                                    image3: filteredImage[categories.indexOf(category)][2], 
                                    isActive: links[categories.indexOf(category)],),
                                );}
                            ),
                            ],
                          );
